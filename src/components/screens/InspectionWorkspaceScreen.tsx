      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4 sm:p-5 bg-slate-900 text-white rounded-3xl shadow-md border border-slate-800">
        ...
        <div className="flex items-center gap-3 text-xs">
          <div className="px-3 py-1.5 bg-slate-800 rounded-xl border border-slate-700">
            ...
            <span className="font-bold text-white">{activeInspectionInstrument.capacity} {activeInspectionInstrument.unitOfMeasurement}</span>
          </div>
          <div className="px-3 py-1.5 bg-slate-800 rounded-xl border border-slate-700">
            <span className="text-slate-400 block text-[10px] uppercase font-bold">Permissible Tolerance</span>
            <span className="font-bold text-cyan-400">±{(activeInspectionInstrument.permissibleTolerance * 100).toFixed(2)}%</span>
          </div>
        </div>
      </div>
      ...
      <div className="p-5 sm:p-6 bg-white rounded-3xl border border-slate-200/90 shadow-xs space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-2 text-xs">
            <span className="font-medium text-slate-500">Observed Max Error:</span>
            <span className={`font-mono font-bold px-2 py-0.5 rounded ${
              maxError <= activeInspectionInstrument.permissibleTolerance
                ? 'bg-emerald-50 text-emerald-700 border border-emerald-200'
                : 'bg-rose-50 text-rose-700 border border-rose-200'
            }`}>
              {(maxError * 100).toFixed(3)}% (Threshold: ±{(activeInspectionInstrument.permissibleTolerance * 100).toFixed(2)}%)
            </span>
          </div>
        </div>
          ...
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200 text-slate-600">
                <th className="py-3 px-3 font-bold"># Test Point</th>
                <th className="py-3 px-3 font-bold">Standard Mass ({activeInspectionInstrument.unitOfMeasurement})</th>
                <th className="py-3 px-3 font-bold">Actual Reading ({activeInspectionInstrument.unitOfMeasurement})</th>
                <th className="py-3 px-3 font-bold">Calculated Error (%)</th>
                <th className="py-3 px-3 font-bold">Statutory Limit</th>
                <th className="py-3 px-3 font-bold">Tolerance Status</th>
                ...
                return (
                  <tr key={reading.id} className="hover:bg-slate-50/70 transition-colors">
                    ...
                    <td className="py-3 px-3 font-mono">
                      <span className={reading.errorPercentage >= 0 ? 'text-slate-800' : 'text-slate-800'}>
                        {reading.errorPercentage >= 0 ? '+' : ''}{(reading.errorPercentage * 100).toFixed(3)}%
                      </span>
                    </td>
                    <td className="py-3 px-3 font-mono text-slate-500">
                      ±{(reading.toleranceLimit * 100).toFixed(2)}%
                    </td>
                    <td className="py-3 px-3">
                      {isPass ? (
                        ...
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-rose-50 text-rose-700 rounded-md font-bold text-[11px] border border-rose-200">
      ...
      <div className="p-5 sm:p-6 bg-gradient-to-br from-slate-900 via-slate-900 to-slate-800 text-white rounded-3xl shadow-lg border border-slate-800 space-y-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-800">
          ...
          <button
            ...
            className="inline-flex items-center justify-center gap-2 px-5 py-2.5 bg-cyan-500 hover:bg-cyan-400 text-slate-950 rounded-xl text-xs font-black shadow-md transition-all active:scale-95 disabled:opacity-50 shrink-0"
          >
            {isAiAnalyzing ? (
              <>
