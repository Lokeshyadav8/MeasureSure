    ...
    <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-slate-200 shadow-xs pt-[max(env(safe-area-inset-top,0px),0.5rem)]">
      ...
      <div className="bg-slate-900 text-slate-200 px-4 py-1 text-[10px] font-bold flex items-center justify-between border-b border-slate-800">
        <div className="flex items-center gap-2 truncate">
          ...
          <span className="hidden sm:inline text-slate-300 truncate">OIML R-76 NAWI Verification | PS SIH26035</span>
        </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-1.5 sm:hidden">
              <a
                ...
                title="Download APK"
                className="inline-flex items-center gap-1 px-2.5 py-1.5 bg-cyan-100 text-cyan-950 border border-cyan-300 rounded-lg text-xs font-black whitespace-nowrap active:scale-95"
              >
              ...
              <button
                onClick={() => runDemoFlow()}
                className="inline-flex items-center gap-1 px-2.5 py-1.5 bg-cyan-600 hover:bg-cyan-700 text-white rounded-lg text-xs font-black shadow-xs whitespace-nowrap active:scale-95"
              >
          ...
          <div className="flex flex-wrap items-center justify-between sm:justify-end gap-2">
            ...
            <div className="hidden sm:flex items-center gap-2">
              <a
                ...
                className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-cyan-100 hover:bg-cyan-200 text-cyan-950 border border-cyan-300 rounded-xl text-xs font-black transition-all active:scale-95 whitespace-nowrap"
              >
            ...
            <div className="flex items-center p-0.5 sm:p-1 bg-slate-100 rounded-xl border border-slate-300 overflow-x-auto no-scrollbar w-full sm:w-auto">
              {roleOptions.map(opt => {
                ...
                return (
                  <button
                    ...
                    className={`inline-flex items-center justify-center gap-1 px-2.5 sm:px-3 py-1 rounded-lg text-xs font-bold transition-all whitespace-nowrap flex-1 sm:flex-initial ${
                      isActive
                        ? 'bg-white text-slate-950 shadow-xs border border-slate-300 font-extrabold'
                        : 'text-slate-700 hover:text-slate-950'
                    }`}
                  >
                    <span className={isActive ? 'text-cyan-700 shrink-0' : 'text-slate-600 shrink-0'}>
                      {opt.icon}
                    </span>
            ...
            {/* User Profile Pill */}
            <div className="hidden lg:flex items-center gap-2 pl-2 border-l border-slate-300">
              <div className="w-8 h-8 rounded-full bg-slate-900 text-cyan-300 font-bold text-xs flex items-center justify-center shadow-xs shrink-0">
                {currentUser.name.split(' ').map(n => n[0]).join('').slice(0, 2).toUpperCase()}
              </div>
              <div className="text-left leading-tight">
                <div className="text-xs font-extrabold text-slate-950 truncate max-w-[130px]">
                  {currentUser.name}
                </div>
                <div className="text-[10px] text-slate-700 font-semibold truncate max-w-[130px]">
